...
export function GoalTemplateTab() {
  ...
  >([
    {
      ...
      prompt:
        'Whisper: "Remember to keep a calm and professional tone, even if the conversation becomes challenging. Focus on finding solutions, not problems."',
    ...
    },
    {
      id: 2,
      type: "business",
      name: "Stay Focused on Key Metrics",
      prompt:
        'Whisper: "Guide the conversation toward the KPIs that matter—mention revenue targets, customer acquisition, and cost savings. Don\'t get sidetracked."',
      tags: ["metrics", "focus"],
      isSystem: true,
      isHidden: false,
      assignedContacts: [],
    },
    {
      id: 3,
      type: "personal",
      name: "Stay Calm and Composed",
      prompt:
        'Whisper: "Take a deep breath. Keep your tone calm and centered, especially if the conversation becomes stressful."',
    ...
    },
    {
      ...
      prompt:
        "Whisper: \"Focus on truly listening. Don't interrupt—acknowledge what's being said before you respond to show empathy.\"",
  ...
  const handleEditTemplate = () => {
    if (editedTemplate.id) {
      if (editedTemplate.isSystem) {
        ...
        setSelectedTemplate({
          ...editedTemplate,
          id: editedTemplate.id ?? Date.now(),
          isHidden: editedTemplate.isHidden ?? false,
        });
        toast({
  ...
  const handleToggleHideTemplate = (templateId: number) => {
    ...
    toast({
      ...
      description: `The template has been ${
        templates.find((t) => t.id === templateId)?.isHidden
          ? "unhidden"
          : "hidden"
      }.`,
    });
  ...
  const handleAddTag = () => {
    if (newTag && !editedTemplate.tags.includes(newTag)) {
      setEditedTemplate({
        ...editedTemplate,
        tags: [...editedTemplate.tags, newTag],
      });
      setNewTag("");
    }
  };
    ...
    <div className="p-8 bg-gray-900 text-white">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400">
          Manage and customize your goal templates for AI-assisted calls
        </p>
      </div>
          ...
          <CardContent>
            <ScrollArea className="h-[calc(100vh-300px)] template-list-scroll-area">
              {filteredTemplates.map((template) => (
                <div
                  key={template.id}
                  className="flex flex-col p-2 mb-2 bg-gray-700 rounded"
                >
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      {template.isSystem ? (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleToggleHideTemplate(template.id)}
                        >
                          {template.isHidden ? (
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    ...
                    {template.assignedContacts.length > 0 && (
                      ...
                      >
                        {template.assignedContacts.length} Contact
                        {template.assignedContacts.length > 1 ? "s" : ""}
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {selectedTemplate.assignedContacts.length > 0 ? (
                      contacts
                        .filter((contact) =>
                          selectedTemplate.assignedContacts.includes(contact.id)
                        )
                        .map((contact) => (
                          <Badge
                            key={contact.id}
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="bg-gray-800 border-gray-700 text-white">
          <DialogHeader>
            <DialogTitle>
              {editedTemplate.id
                ? editedTemplate.isSystem
                  ? "Create User Template from System Template"
                  : "Edit User Template"
                ...
                {currentContacts.map((contact) => (
                  <div key={contact.id} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      checked={editedTemplate.assignedContacts.includes(
                        contact.id
                      )}
