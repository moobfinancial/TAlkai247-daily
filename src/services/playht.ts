...
export const playhtApi = {
  ...
  async getVoices(): Promise<PlayHTVoice[]> {
    try {
      ...
      const response = await axios.get('/api/voices/playht/voices');
  ...
  async generateSpeech(text: string, voiceId: string): Promise<ArrayBuffer> {
    try {
      ...
      const response = await axios.post('/api/voices/playht/speech', {
        text,
        voice: voiceId,
        quality: 'premium',
        output_format: 'mp3'
  ...
  async previewVoice(voiceId: string): Promise<ArrayBuffer> {
    try {
      ...
      const response = await axios.get(`/api/voices/playht/preview?url=${encodeURIComponent(previewUrl)}`, {
        responseType: 'arraybuffer'
      });
